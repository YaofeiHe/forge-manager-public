...
This public skill is generated from the repository skill source. Install the Python package first, then use the Codex plugin or repo-scoped skill. Replace `<PROJECT_ROOT>` and other placeholders with the cloned repository or target project paths available on the current host. Do not copy private local credential paths, tokens, cookies, browser profiles, or host-specific runtime artifacts into public docs.
